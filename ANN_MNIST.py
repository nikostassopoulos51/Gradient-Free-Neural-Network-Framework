...
def clustering(neurons, xx_clust, max_iter=300):
    kmeans = KMeans(n_clusters=neurons, max_iter=max_iter, n_init=20, random_state=51)  # Increased n_init for stability
    kmeans.fit(xx_clust)
...
def train_layer_1_sigmoid(neurons, vars, obs, n_per_part, inds_all, x, y):
    a_all = []
    y = np.clip(y, 0.01, 0.99)  # Prevents extreme values in isigm1
    ...
    for i in range(neurons):
        try:
            ...
            atmp = atmp.flatten()  # Ensures correct (vars + 1,) shape
            
            if atmp.shape[0] == vars + 1:
                a_all.append(atmp)
            else:
                a_all.append(np.zeros(vars + 1))  # Fallback for incorrect shapes
    ...
    print(f"Expected shape: {expected_size}, Actual shape: {actual_size}")
...
def fit_nfolds_sigmoid(xx_train, yy_train, nof_folds, vars, i_train, i_fold, neurons):
    np.random.seed(None)  # Ensures different initialization each time
...
def predict(x_input, a_all, a_layer1):
    ...
    probabilities = softmax(output)  # Use softmax instead of sigmoid
...
x_train_scaled = scaler.fit_transform(x_train_flat)  # Standardize
...
random_idx = np.random.randint(0, len(x_train_scaled))  # Random index
